...
Requires CMake ≥ 3.15 and any C++17 compiler. All commands below run from this
`inference-engine/` directory.
...
```mermaid
graph TD
    Serving["serving"] --> Runtime["runtime"]
    Runtime --> Providers["providers"]
    Runtime --> Mdl["model"]
    Providers --> Kernels["kernels"]
    Providers --> Grph["graph"]
    Mdl --> Grph
    Mdl --> Core["core"]
    Kernels --> Core
    Grph --> Core
...
`GraphOptimizer` is the one component that reaches sideways: it lives in `graph`
but takes an `ExecutionProvider&` so constant folding can evaluate a subgraph
with the same kernels inference will use.
